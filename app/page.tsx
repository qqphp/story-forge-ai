...
import type { AppSettings, Asset, BackgroundMusic, PromptTemplate, VoiceItem, Workflow, WorkspacePage } from "@/app/features/shared/types";
...
import { MultiPublishCenterPage } from "@/app/features/publishing/MultiPublishCenterPage";
import { coverSizeGroups } from "@/app/features/prompts/cover-size-groups";
import { FormatSelect, ModelSelect, SpeechRateControl, VoiceControl } from "@/app/features/settings/SelectionControls";
import { defaultSettings, speechFormats } from "@/app/features/settings/defaults";
...
function ImageSizePicker({value,onChange}:{value:string[];onChange:(value:string[])=>void}) {
  const hasRequiredVideoCoverSizes=value.includes("16:9")&&value.includes("9:16");
  return <fieldset className="image-size-picker"><legend>图片尺寸 <em>*</em></legend><p>可多选；每个比例将单独生成一张封面图片</p><div>{coverSizeGroups.map(group=><section key={group.label}><b>{group.label}</b><div>{group.options.map(([size,label])=><label key={size}><input type="checkbox" checked={value.includes(size)} onChange={event=>onChange(event.target.checked?[...value,size]:value.filter(item=>item!==size))}/><span>{label}</span></label>)}</div></section>)}</div>{!hasRequiredVideoCoverSizes&&<small className="image-size-required">请同时选择 16:9 和 9:16 图片尺寸</small>}<small className="image-size-note">由于中转站的 gpt-image-2 接口实际调用 gpt-image-2-codex，同一条提示词需按比例分别调用一次接口才能生成一张图片；该接口不支持设置分辨率参数，仅会在提示词中追加图片比例。</small><small className="image-size-note">视频封面默认使用 16:9 或者 9:16 比例的图片。</small></fieldset>;
}
...
function PromptLibraryDialog({connected,onClose,onSaved}:{connected:boolean;onClose:()=>void;onSaved:()=>void}) {
  ...
  const [items,setItems]=useState<PromptTemplate[]>([]); const [name,setName]=useState(""); const [text,setText]=useState(""); const [imageSizes,setImageSizes]=useState<string[]>(["16:9","9:16"]);
  ...
  const clearEditor=()=>{setName("");setText("");setImageSizes(["16:9","9:16"]);setEditingId(null)};
  const hasRequiredCoverSizes=imageSizes.includes("16:9")&&imageSizes.includes("9:16");
  const saveTemplate=async()=>{if(!name.trim()||!text.trim()||(kind==="cover"&&!hasRequiredCoverSizes))return;setSaving(true);try{const url=editingId?`${API}/api/prompts/${editingId}`:`${API}/api/prompts`;const payload={name,text,image_sizes:imageSizes};const res=await fetch(url,{method:editingId?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(editingId?payload:{kind,...payload})});if(res.ok){clearEditor();load();onSaved();}}finally{setSaving(false)}};
...
function DetailPanel({task,onClose,onRetry,onDelete}:{task:Workflow;onClose:()=>void;onRetry:()=>void;onDelete:()=>Promise<void>}) {
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><aside className="drawer">
    ...
    <div className="drawer-content">
      ...
      {tab==="videos"&&<section className="video-grid">{task.videos?.length?task.videos.map((a,i)=><figure key={i}><video controls preload="metadata" src={`${API}${a.url}`}/><figcaption><b>{a.voice} · 分享稿 {i+1}</b><span>分辨率：{a.resolution||"未记录"}</span><span>视频时长：{formatVideoDuration(a.duration)}</span></figcaption></figure>):<EmptyMedia text="视频尚未生成"/>}</section>}
...
function formatVideoDuration(value?:number) {
  if(value===undefined||value===null||!Number.isFinite(value))return "未记录";
  ...
  return <div className="video-settings-page"><div className="page-section-head"><div><p className="eyebrow">VIDEO WORKFLOW</p><h1>视频设置</h1><p>统一控制画面方向和视频素材来源；新启动的工作流会使用这里的配置。</p></div></div>
    <section className="settings-pane video-settings-pane">
      <div className="settings-block"><h3>画面方向</h3><p className="setting-explainer">合成时会自动选择同方向封面，并输出对应分辨率。</p><div className="video-option-grid">
        <label className={form.video_orientation==="landscape"?"selected":""}><input type="radio" name="video-orientation" checked={form.video_orientation==="landscape"} onChange={()=>setForm({...form,video_orientation:"landscape"})}/><span><b>横版 16:9</b><small>输出 1920 × 1080，使用 16:9 封面</small></span></label>
        <label className={form.video_orientation==="portrait"?"selected":""}><input type="radio" name="video-orientation" checked={form.video_orientation==="portrait"} onChange={()=>setForm({...form,video_orientation:"portrait"})}/><span><b>竖版 9:16</b><small>输出 1080 × 1920，使用 9:16 封面</small></span></label>
      </div></div>
      <div className="settings-block"><h3>生成方式</h3><div className="video-option-grid">
        ...
        <label className={form.video_generation_method==="stock"?"selected":""}><input type="radio" name="video-method" checked={form.video_generation_method==="stock"} onChange={()=>setForm({...form,video_generation_method:"stock"})}/><span><b>引用无版权视频</b><small>搜索 3 段 1080p 素材并循环至配音结束</small></span></label>
      </div></div>
