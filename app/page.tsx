...
function VoiceSettingsDialog({connected,onClose,onSaved}:{connected:boolean;onClose:()=>void;onSaved:()=>void}) {
  ...
  const [music,setMusic]=useState<BackgroundMusic[]>([]); const [musicTotal,setMusicTotal]=useState(0); const [musicPage,setMusicPage]=useState(1); const [musicQuery,setMusicQuery]=useState(""); const [musicForm,setMusicForm]=useState({name:"",url:"",category:""}); const [musicError,setMusicError]=useState("");
  ...
  const save=async()=>{if(!connected){onSaved();return;}setSaving(true);try{const response=await fetch(`${API}/api/settings`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(form)});if(!response.ok)throw new Error("保存失败");onSaved();}finally{setSaving(false)}};
  const preview=async(voice:string)=>{setPreviewing(voice);try{const audio=new Audio(`${API}/api/voices/${encodeURIComponent(voice)}/preview`);await audio.play();audio.onended=()=>setPreviewing("");audio.onerror=()=>setPreviewing("");}catch{setPreviewing("")}};
  ...
  const addMusic=async()=>{setMusicError("");if(!musicForm.name.trim()){setMusicError("请填写背景音乐名称");return;}if(!/^https:\/\/.+/i.test(musicForm.url.trim())){setMusicError("请输入有效的 HTTPS 音频地址");return;}const response=await fetch(`${API}/api/background-music`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(musicForm)});if(!response.ok){setMusicError("添加失败，请检查填写内容");return;}setMusicForm({name:"",url:"",category:""});setMusicPage(1);void loadMusic()};
  const removeMusic=async(id:string)=>{await fetch(`${API}/api/background-music/${id}`,{method:"DELETE"});void loadMusic()};
