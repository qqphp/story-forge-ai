(function exposeCoverUploadHelpers(root){
  ...
  function activateBilibiliCoverRegion(region){
    ...
    const bounds=target.getBoundingClientRect?.()||{left:0,top:0,width:0,height:0};
    const options={bubbles:true,cancelable:true,composed:true,button:0,buttons:1,clientX:bounds.left+bounds.width/2,clientY:bounds.top+bounds.height/2};
    const PointerEventType=root.PointerEvent||root.MouseEvent;
    if(PointerEventType)target.dispatchEvent(new PointerEventType("pointerdown",{...options,pointerId:1,pointerType:"mouse",isPrimary:true}));
    target.dispatchEvent(new root.MouseEvent("mousedown",options));
    if(PointerEventType)target.dispatchEvent(new PointerEventType("pointerup",{...options,buttons:0,pointerId:1,pointerType:"mouse",isPrimary:true}));
  ...
  root.StoryForgeCoverUpload={imageFormatFromBytes,pickImageInput,activateBilibiliCoverRegion};
})(globalThis);
