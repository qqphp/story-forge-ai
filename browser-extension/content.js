...
(async()=>{
  ...
  function findImageInput(){return [...document.querySelectorAll('input[type="file"]')].find(input=>/image|png|jpe?g|webp/i.test(input.accept||""))||null}
  ...
  async function uploadCoverAsset(coverIndex,cover,slotIndex){
    showMessage(`正在上传${cover.image_ratio}封面并等待抖音裁剪窗口…`);
    ...
    const blob=await response.blob();const filename=decodeURIComponent(cover.url.split("/").pop()||"storyforge-cover.png");const file=new File([blob],filename,{type:blob.type||"image/png"});
    const trigger=await waitFor(()=>{const choices=exactTextElements("选择封面");return choices[slotIndex]||choices[0]},30000);if(!trigger)throw new Error(`没有找到抖音的${cover.image_ratio}封面入口`)
    const previousImageInputs=new Set([...document.querySelectorAll('input[type="file"]')].filter(input=>/image|png|jpe?g|webp/i.test(input.accept||"")));
    trigger.click();await new Promise(resolve=>setTimeout(resolve,500));
    const imageInputs=[...document.querySelectorAll('input[type="file"]')].filter(candidate=>/image|png|jpe?g|webp/i.test(candidate.accept||""));
    let input=imageInputs.find(candidate=>!previousImageInputs.has(candidate))||imageInputs.at(-1)||findImageInput();
    if(!input){for(const label of ["上传封面","本地上传","上传图片"]){if(clickExactText(label)){input=await waitFor(findImageInput,10000);break}}}
    if(!input)throw new Error("已打开封面设置，但没有识别到图片上传控件，请先手动选择“上传封面”")
    assignFile(input,file);await new Promise(resolve=>setTimeout(resolve,1200));
    const dialog=input.closest('[role="dialog"],[class*="modal"],[class*="Modal"]')||document;
    for(const label of ["完成","确定","确认","保存"]){const button=exactTextElements(label,dialog)[0];if(button){button.click();break}}
  ...
  async function attachCovers(){
    ...
    const uploaded=[];
    for(const target of [{ratio:"4:3",slotIndex:1},{ratio:"3:4",slotIndex:0}]){
      const coverIndex=covers.findIndex(cover=>cover.image_ratio===target.ratio);if(coverIndex<0)continue;
      await uploadCoverAsset(coverIndex,covers[coverIndex],target.slotIndex);uploaded.push(target.ratio);
    }
  ...
  async function fillTask(){
    fillButton.disabled=true;try{
      ...
      showMessage(`已填充标题和简介${topicStatus}${coverStatus}。请检查裁剪结果、可见范围等设置，然后亲自点击抖音的发布按钮。`,topics.failed.length>0||coverResult.skipped.length>0);
